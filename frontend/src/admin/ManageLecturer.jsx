...
function ManageLecturer() {
  ...
  const filteredLecturers = useMemo(() => {
    return lecturers?.filter(lecturer => {
      const matchesSearch = 
        ...
        lecturer.position?.join("").toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleCreateLecturer = async () => {
    try {
      ...
      if (profileImage) {
        formData.append('profileImageURL', profileImage); // Changed from 'profileImage' to 'image'
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">View and manage all lecturers in M.S.T</p>
        </div>
