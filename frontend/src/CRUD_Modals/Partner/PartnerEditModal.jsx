...
//             className="px-4 py-2 bg-[#FFC53A] text-gray-900 rounded-lg hover:bg-[#e6b234] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
//           >
//             Save Changes
//           </button>
//         </div>
...
function PartnerEditModal({ showEditModal, setShowEditModal, selectedPartner, setSelectedPartner, partnerTiers, availableCategories, handleEditPartner }) {
  ...
  // Sync preview when modal opens or partner changes
  useEffect(() => {
    if (selectedPartner?.logo) {
      // If it's a string (URL from DB), use it. If it's a File object, create a blob.
      const url = typeof selectedPartner.logo === 'string' 
        ? selectedPartner.logo 
        : URL.createObjectURL(selectedPartner.logo);
      setPreviewUrl(url);
    } else {
      setPreviewUrl(null);
    }
