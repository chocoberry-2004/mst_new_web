...
function ApplicationForm() {
  const { showModal, ApplicationFormHandler, formType } = useContext(AppContext);
  const { facultyList, facultyLoading } = useFaculty();
  const { events } = useEventContext();
  const { course } = useCourse();

  // { label: c.name, value: `faculty-${c.id}` }

  const InterestedOptions = {
    program : facultyList?.faculty?.courses?.map(c => ({ label: c.name, value: `faculty-${c.id}` })) || [] ,
    course  : course?.courses?.map(c => ({ label: c.name, value: `course-${c.id}` })) || [] ,
    event   : events?.map(e => ({ label: e.title, value: `event-${e.id}` })) || [] ,
  ...
  return (
    <div 
      ...
      className={`bg-black/60 backdrop-blur-sm transition-all duration-300 ease-in-out justify-center fixed inset-0 top-0 w-full h-screen z-[100] items-center p-5 ${showModal ? 'flex' : 'hidden'}`}
    >
      <div 
        onClick={(e) => e.stopPropagation()}
        className="bg-white rounded-lg min-w-150 border border-gray-300 max-h-[100vh] overflow-y-auto scrollbar-hide"
      >
        {/* Header */}
        ...
        <form className="space-y-6 p-5">
          ...
          <div>
            ...
            <select 
              ...
              required
            >
              <option value="">Select a {formType}</option>
              {InterestedOptions[formType]?.map((opt) => (
